...
{
    /// <summary>
    /// GOAP (Goal-Oriented Action Planning) Planner that helps AI agents decide what actions to take.
    /// 
    /// How it works:
    ...
    {
        [Header("- Debugging")]
        [Tooltip("Enable this to see detailed planning information in the console")]
        [SerializeField] private bool showDebugLogs = true;
        [Tooltip("Stop planning if we can't find a solution after this many tries")]
        [SerializeField] private int maxSearchIterations = 1000;
        ...
        public List<GoapAction> CreatePlan(GladiatorAgent agent, List<GoapAction> availableActions, Dictionary<WorldStateKey, bool> currentState, Dictionary<WorldStateKey, bool> goalState)
        {
            if (showDebugLogs)
            ...
            int iterations = 0;
            while (statesToExplore.Count > 0 && iterations < maxSearchIterations)
            {
                iterations++;
        ...
        /// </summary>
        private bool HasReachedGoal(Dictionary<WorldStateKey, bool> currentState, Dictionary<WorldStateKey, bool> goalState)
        {
        ...
        /// </summary>
        private float CalculateHeuristic(Dictionary<WorldStateKey, bool> currentState, Dictionary<WorldStateKey, bool> goalState)
        {
            float distance = 0;
            foreach (var goal in goalState)
            {
        ...
        /// </summary>
        private Dictionary<WorldStateKey, bool> ApplyActionEffects(Dictionary<WorldStateKey, bool> currentState, GoapAction action)
        {
